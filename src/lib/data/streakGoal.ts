/* Streak goals and gentle achievements (phase 4 features ticket 20,
   ...
   "Achievement" rather than "milestone" throughout, on purpose: CONTEXT.md
   already defines Milestone as a dated significant day kept separately
   from entries, and an achievement rung here is a streak-length threshold
   - a different concept that happens to share a common English word with
   it, not a second sense of the same domain term.
...
/** Streak lengths a gentle achievement calls out, in days. Not a reward
    ladder that can be lost: each length reachedAchievements() reports is
...
export const GOAL_ACHIEVEMENT_DAYS = [3, 7, 14, 30, 60, 100, 180, 365] as const;
...
/** Target lengths offered when setting a goal - a short list to choose
    ...
    GOAL_ACHIEVEMENT_DAYS rung, so hitting the target is itself the first
...
/** Which rungs of GOAL_ACHIEVEMENT_DAYS the longest streak on record has
    ever reached. */
export function reachedAchievements(bestStreakEver: number): number[] {
  return GOAL_ACHIEVEMENT_DAYS.filter((days) => bestStreakEver >= days);
}
