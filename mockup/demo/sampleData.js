/* Sample demo data — one coherent persona: Alice, ~2.5 years into transition.
   All dates are generated relative to "today" with a seeded PRNG, so the demo
   is deterministic (Reset always restores exactly this state) but never stale. */

const DAY = 86400000;
export const todayEpochDay = () => Math.floor((Date.now() - new Date().getTimezoneOffset() * 60000) / DAY);

/* Deterministic PRNG (mulberry32) — same data every seed. */
function rng(seed) {
  return function () {
...
/* Notes pool — warm, real, varied length. Indexed deterministically. */
const NOTES = [
...
function buildEntries() {
  ...
  const entries = [];
  let id = 1;
  // ~150 days of history; denser recently; last 23 days unbroken (streak).
  for (let back = 150; back >= 0; back--) {
    const day = today - back;
    const isStreak = back <= 22;
    const logged = isStreak || r() < 0.72;
    if (!logged) continue;
    const nEntries = back === 0 ? 1 : (r() < 0.12 ? 2 : 1);
    // Gentle upward drift over time + noise; euphoria correlates with mood.
    ...
    for (let k = 0; k < nEntries; k++) {
      ...
      const photos = r() < 0.10
        ? [{ id: 'ph' + id, hue: Math.floor(r() * 360), label: 'Photo' }]
        : [];
      // Today's sample entry sits a few hours in the past, so anything the
      // reviewer logs "now" always sorts above it.
      const ts = back === 0
        ? Date.now() - 3 * 3600000
        : (day * DAY) + hour * 3600000 + minute * 60000;
      entries.push({
        id: id++,
        epochDay: day,
        timestamp: ts,
...
export function initialState() {
  ...
  return {
    version: 1,
    prefs: {
      onboarded: true,          // Reset → intro chooser offers "first run" walk
      name: 'Alice',
      activePreset: 'p-btw',
      colorMetric: 'mood',      // 'mood' | dimension key
      theme: 'light',           // system | light | dark (mockup: light/dark via control bar too)
      palette: 'trans',
      language: 'en',           // system | en | pl
      appLock: false,
      lockOnLeave: false,
      disguise: false,
      quickExit: false,
      ...
      lastBackupAt: (today - 34) * DAY,   // 34 days ago → stale-backup notice
    ...
    milestones: [
      { id: 'm1', name: 'HRT start', epochDay: today - 745, kind: 'anniversary', templateKey: 'hrt_start', photo: { hue: 205, label: 'Photo' } },
      { id: 'm2', name: 'Coming out to my parents', epochDay: today - 940, kind: 'anniversary', templateKey: 'coming_out', photo: null },
      { id: 'm3', name: 'First time presenting publicly', epochDay: today - 512, kind: 'anniversary', templateKey: 'first_public', photo: { hue: 330, label: 'Photo' } },
